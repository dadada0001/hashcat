...
KERNEL_FQ KERNEL_FA void m76020_mxx (KERN_ATTR_VECTOR_ESALT (cryptojs_t))
{
  ...
  for (u32 il_pos = 0; il_pos < IL_CNT; il_pos += VECT_SIZE)
  {
    ...
    // CBC XOR with derived IV (single block, data_len == 16)
    ...
    // empty plaintext check: full PKCS7 padding = 0x10101010 in all 4 words

    if (dec[0] != 0x10101010) continue;
    if (dec[1] != 0x10101010) continue;
    if (dec[2] != 0x10101010) continue;
    if (dec[3] != 0x10101010) continue;
...
KERNEL_FQ KERNEL_FA void m76020_sxx (KERN_ATTR_VECTOR_ESALT (cryptojs_t))
{
  const u64 gid = get_global_id (0);
  const u64 lid = get_local_id (0);
  const u64 lsz = get_local_size (0);
